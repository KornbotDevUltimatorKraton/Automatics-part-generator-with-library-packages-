...
os.system("echo 'Rkj3548123' | sudo -S mkdir Packagelibrary")
...
os.system("echo 'Rkj3548123' | sudo -S chmod -R 777 Packagelibrary") #Activate permission for the package library directory
...
def Paringtables(inputcomp,Predictbreak):  #Bredict the breaking data 
        ...
        if len(Getpagevalue) <=1: 
                ...
                for rl in range(0,len(Getpagevalue)):
                            os.system("echo 'Rkj3548123' | sudo -S mkdir"+"\t"+EXTRACT+"/"+str(inputcomp)+"/"+str(inputcomp))
                            os.system("echo 'Rkj3548123' | sudo -S chmod -R 777"+"\t"+EXTRACT+"/"+str(inputcomp))
                            os.system("echo 'Rkj3548123' | sudo -S cp"+"\t"+EXTRACT+"/"+str(inputcomp)+".csv"+"\t"+"-t"+"\t"+EXTRACT+"/"+str(inputcomp)+"/"+str(inputcomp)) #Getting the number of the pins on the package data
        if len(Getpagevalue) >1:
...
def Packagefor_3dlibclass(input1,inputcomp,listConfig):
       ...
       for i in reversed(range(0,input1.getNumPages())):  # Running the page for the back checking 
              ...
              os.system("echo 'Rkj3548123' | sudo -S mkdir"+"\t"+EXTRACT+"/"+inputcomp)
              os.system("echo 'Rkj3548123' | sudo -S chmod -R 777"+"\t"+EXTRACT+"/"+inputcomp)   
       ...
       os.system("echo 'Rkj3548123' | sudo -S mkdir"+"\t"+EXTRACT+"/"+inputcomp+"/"+inputcomp+"R"+"Pinsfunc")
       os.system("echo 'Rkj3548123' | sudo -S chmod -R 777"+"\t"+EXTRACT+"/"+inputcomp+"/"+inputcomp+"R"+"Pinsfunc")    
       ...
       if datalist != []: #Checking if the directory is not blank  
           ...
           for il in range(0,len(listrecorder)):
                       ...
                       if checkmatch !=[]:
                              ...
                              if percent.ratio()*100 >= 85.71428571428571 and percent.ratio()*100 <= 100.0: 
                                       ...
                                       os.system("echo 'Rkj3548123' | sudo -S cp"+"\t"+EXTRACT+"/"+inputcomp+"/"+inputcomp+"_"+str(getpage)+".csv"+"\t"+"-t"+"\t"+PINSFUNC) #Moving the file extracted into the pins function 
           ...
           if datalist2 != []: #Checking if the directory is not blank  
              ...
              """
              for ir in range(0,len(sorting2)):
                     print("Data frame from page:",sorting2[ir]) #Getting page number of the sorting list 
                     df = pd.read_csv(PINSFUNC+"/"+inputcomp+"_"+str(sorting[ir])+".csv") #Getting the data frame from the list csv file
                     print(df) #Getting the dataframe of the csv file 
                     listdataindex = list(df.columns.values) 
                     print("Reading header columns for matching check:",listdataindex) #getting the data index 
                     print("Begin recording data of header")
                     recordheader2[str(sorting[ir])] = listdataindex #Record data frame
              print("Recorded data2 from the header columns")
              print("Record data2 function",recordheader2) 
              """ 
           #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
           #Matching sequence 
           listrecorder = list(recordheader) 
...
SpecificationExtract(input1, searchSpecification) #Specification page extraction
print("spec page number:",Specpage) # Reference for the specpage  
Classifyselectionfunction(input1,inputcomp,Pinsquantity,Packagecheck,len(reftabledetect),combinedictdata)
Devicenameparing(combinedictdata)#Device paring function 
Groupingdevicelist(Deviceeven,Deviceodd)
...
if len(list(Predictbreak)) >1: 
       precisecuthead(Pathdata)
       Mergecsvalgorithm(Pathdata)
Editingxmlpreprocess(inputcomp,listConfig) #Getting the input for xml editor 
Checkinginsidedirectory(inputcomp)
